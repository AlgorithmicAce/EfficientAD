...
def get_argparse():
    ...
    parser.add_argument('-d', '--dataset', default='VisA',
                        choices=['VisA'])
    parser.add_argument('-s', '--subdataset', default='pcb2',
                        help='One of 15 sub-datasets of Mvtec AD or 5' +
                             'sub-datasets of Mvtec LOCO')
    parser.add_argument('-o', '--output_dir', default='visa_output/1')
    ...
    parser.add_argument('-a', '--visa_path',
                        default='./VisA',
                        help='VisA datapath')
...
def main():
    ...
    bad_test_set = ImageFolderWithPath(
        os.path.join(dataset_path, config.subdataset, 'Data', 'Images','Normal'))
    print("Configured bad test data")
    if config.dataset == 'VisA':
        # 80% for training data, 10% for validation data, 10% for testing data
        train_size = int(0.8 * len(full_train_set))
        validation_size = int(0.1 * len(full_train_set))
